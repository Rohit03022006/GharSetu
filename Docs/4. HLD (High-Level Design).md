...
architecture-beta
    ...
    group svc(server)[Microservices Layer]
    group cache(database)[Shared Cache and Events]
    group ext(internet)[External Integrations]

    service web(server)[React SPA] in client

    service gw(server)[Nginx Gateway] in edge

    service identity(server)[Identity Service] in svc
    service identitydb(database)[identity db] in svc
    service listing(server)[Listing Service] in svc
    service listingdb(database)[listing db] in svc
    service finance(server)[Finance Service] in svc
    service financedb(database)[finance db] in svc
    service preference(server)[Preference Service] in svc
    service preferencedb(database)[preference db] in svc
    service discovery(server)[Discovery Service] in svc
    service discoverydb(database)[discovery db] in svc
    service engagement(server)[Engagement Service] in svc
    service engagementdb(database)[engagement db] in svc
    service analytics(server)[Analytics Service] in svc
    service analyticsdb(database)[analytics db] in svc

    service redis(database)[Redis Cache Pub Sub] in cache
    service storage(disk)[MinIO Object Storage] in cache

    service google(internet)[Google OAuth2] in ext
    service osm(internet)[OpenStreetMap] in ext
    service smtp(internet)[SMTP] in ext
    service sms(internet)[SMS Gateway] in ext
    ...
    identity:B -- T:identitydb
    listing:B -- T:listingdb
    finance:B -- T:financedb
    preference:B -- T:preferencedb
    discovery:B -- T:discoverydb
    engagement:B -- T:engagementdb
    analytics:B -- T:analyticsdb

    listing:R -- L:redis
    preference:R -- L:redis
    discovery:R -- L:redis
    engagement:R -- L:redis
    analytics:R -- L:redis

    identity:T -- B:storage
    listing:T -- B:storage
